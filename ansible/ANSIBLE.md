...
## Install Ansible & quick test

The following are short, copy-paste steps to install Ansible and run quick verifications locally. Tailor to your OS; the commands below assume a Debian/Ubuntu host or macOS where noted.
...
### Quick verification (syntax, lint, dry-run)
...
2) Optional: lint the playbook (if you have ansible-lint installed)
...
```bash
ansible-playbook --check -i ansible/hosts.ini ansible/playbook.yml
```

This uses `ansible/hosts.ini` (edit or replace with a test inventory for a VM or cloud host).

### Run against a test VM (Multipass example)

1) Create a disposable VM:

```bash
multipass launch --name cloud-test --mem 2G --disk 10G 22.04
```
...
```bash
multipass list
# or: multipass info cloud-test --format json | jq -r '.[].info."ipv4"[0]'
```

3) Create an inventory file (`ansible/inventory_cloud-test.ini`) and replace <IP>:
...
4) Run syntax check, a dry-run, then apply for real when ready:

```bash
ansible-playbook --syntax-check -i ansible/inventory_cloud-test.ini ansible/playbook.yml
ansible-playbook --check -i ansible/inventory_cloud-test.ini ansible/playbook.yml
ansible-playbook -i ansible/inventory_cloud-test.ini ansible/playbook.yml
```

Notes:
- If SSH fails, check firewalls/security groups, the SSH user name (Ubuntu images use `ubuntu`), and that the private key path is correct. Use `-vvvv` to debug.
- If a target lacks Python 3, set `ansible_python_interpreter=/usr/bin/python` or install Python on the target first.

---

If you want, I can also add a short helper script (`tools/test-with-multipass.sh`) to automate the Multipass steps and generate the inventory. Tell me and I'll add it.

## Automated Multipass test script

There's a helper script in `tools/test-with-multipass.sh` that automates creating a disposable Multipass VM, transferring this repository to the VM, installing Ansible inside the VM, running a syntax check and a dry-run, and optionally applying the playbook or destroying the VM.

Usage (from repo root):

```bash
# create VM and run syntax-check + dry-run (default)
./tools/test-with-multipass.sh create

# run the playbook for real on the VM (DANGEROUS: will install packages)
./tools/test-with-multipass.sh apply

# destroy the VM and remove transferred files
./tools/test-with-multipass.sh destroy

# override VM name or Ubuntu release
./tools/test-with-multipass.sh create my-vm-name 20.04
```

The script logs progress and exits non-zero if a step fails. It requires `multipass` installed on your machine and that your user can run `multipass` commands.

If you prefer, you can follow the manual Multipass steps earlier in this document instead of using the script.
